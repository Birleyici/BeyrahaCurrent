<template>
  <UAccordion
    :items="[{ label: props.item.attribute_name, slot: 'attr-content' }]"
  ...
  >
        ...
        <template #trailing>
          <div class="flex items-center">
            <UIcon
              ...
              class="text-red-500"
              @click.prevent="deleteAttr(props.item.product_attribute_id)"
            >
            </UIcon>
            <UIcon
              name="i-heroicons-chevron-right-20-solid ml-4 "
    ...
    <template #attr-content>
      <div class="border p-minimal rounded-md">
        ...
        <UiFormCheckbox
          class="mt-4"
          v-model="props.item.useForVariation"
          :value="true"
          :id="props.item.attribute_id"
        >
          Varyasyonlar için kullan</UiFormCheckbox
        >
        <UiFormCheckbox
          class="mt-1"
          v-model="props.item.showProductPage"
          :value="true"
          :id="props.item.attribute_id + Math.floor(Math.random() * 10)"
