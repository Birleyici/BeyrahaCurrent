<template>
  <div>
    <div>
      <div v-for="attribute in props.attrsAndVarsState.attributes" :key="attribute.name">
        <!-- Öznitelik Adı -->
        <div class="mb-4">
          <div class="my-minimal" v-if="attribute.name.toLowerCase() == 'renk'">
            <div>
              ...
              <UCarousel
                v-if="$mainState.isLoaded"
                v-slot="{ item }"
                :items="attribute.options"
                :ui="{ item: 'snap-end' }"
              >
                <NuxtImg
                  @click="selectOption(attribute.name, item.term_name, item)"
                  :src="getTermImageSrc(item, item.term_name)"
                  width="60"
                  class="cursor-pointer m-1 rounded-full p-[2px] border w-16 h-16 object-cover object-top"
                  :class="{
    ...
    <div class="my-minimal">
      <div>
        <div v-if="getSelectedVariation">
          <span
            v-if="getSelectedVariation.sale_price"
            class="flex items-center space-x-2"
          >
            <del class="text-gray-500">{{ getSelectedVariation.price }} TL</del>
            <span class="text-secondary-500 text-3xl font-bold"
              >{{ getSelectedVariation.sale_price }} TL</span
            >
          </span>
        ...
        <div v-else>
          <span
            v-if="props.productState.product.sale_price"
            class="flex items-center space-x-2"
          >
            <del class="text-gray-500">{{ props.productState.product.price }} TL</del>
            <span class="text-secondary-500 text-3xl font-bold"
              >{{ props.productState.product.sale_price }} TL</span
            >
          </span>
          <span v-else class="text-secondary-500 text-3xl font-bold">
