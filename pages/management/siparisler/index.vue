<template>
    <div class="space-y-6">
        <!-- Page Header -->
        <AdminCommonPageHeader title="Sipariş Yönetimi" description="Tüm siparişlerinizi görüntüleyin ve yönetin"
            ...
            <div
                ...
                <div class="flex flex-col lg:flex-row gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <USelectMenu v-model="orderState.filters.statuses" :options="orderState.statusOptions" multiple
                            placeholder="Durum seçin..." option-attribute="label" value-attribute="value"
                            class="w-full">
                            <template #label>
        ...
        <div v-if="!orderState.loading" class="hidden lg:block">
            <div
                ...
                <!-- Tablo İçeriği -->
                <div class="p-6">
                    <UTable class="w-full" :columns="orderState.orderListColumns" :rows="orderState.vendorOrders" :ui="{
                        th: {
                        ...
                        <template #status-data="{ row }">
                            <div class="flex space-x-2">
                                ...
                                <UBadge :ui="{ rounded: 'rounded-full' }" color="secondary" v-if="row.shipping_code">
                                    {{ row.shipping_code }}
                                </UBadge>
                            </div>
                        </template>
                        ...
                        <template #actions-data="{ row }">
                            <div class="flex space-x-2">
                                <UButton @click="deleteHandling(row.id)" icon="i-heroicons-trash" size="sm" color="red"
                                    variant="soft" class="hover:bg-red-100 dark:hover:bg-red-900/30" />
                                <UButton v-if="row.status == 'processing'" icon="i-heroicons-qr-code" color="secondary"
                                    variant="soft" :loading="row.shippingLoading"
                ...
                <div class="p-4">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <UBadge :ui="{ rounded: 'rounded-full' }" :color="orderState.statuses[order.status].color">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <!-- Kargo Kodu Al -->
                            <UButton v-if="order.status == 'processing'" icon="i-heroicons-qr-code" color="secondary"
                                variant="soft" :loading="order.shippingLoading"
                                @click="orderState.getShippingCode(order)" size="sm"
                                :ui="{ rounded: 'rounded-full' }" />

                            <!-- Sil -->
                            <UButton @click="deleteHandling(order.id)" icon="i-heroicons-trash" size="sm" color="red"
